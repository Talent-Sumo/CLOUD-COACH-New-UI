...
const BasicCertificate = () => {
    ...
    const basicCertificateRef = useRef();

    return (
        <>
            <Container maxWidth="lg">
                <Box mt={2} mb={2} sx={{ display: "flex", justifyContent: "end" }}>
                    <Button onClick={() => exportComponentAsPNG(basicCertificateRef)} variant='contained' color='secondary'>
                        Download Certificate
                    </Button>
                </Box>
                <Divider />
                <Box sx={{ display: "flex", justifyContent: "center" }}>
                    <Box ref={basicCertificateRef} mt={2} mb={3} p={3} className={styles.certificate}>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={12} sm={12} md={6} lg={6} sx={{ margin: "auto" }}>
                                <img src={basic} alt="basic" style={{ width: '70%' }} />
                            </Grid>
                            <Grid item xs={12} sm={12} md={6} lg={6} sx={{ margin: "auto" }}>
                                <Typography variant='h5' align='end'>
                                    Congratulations, you are awarded a business <span style={{ color: "#1976d2" }}>Basic</span> level recognition for consistently completing 10+ workplace simulations.
